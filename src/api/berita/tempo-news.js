...
const EXTERNAL_BASE_URL = 'https://berita-indo-api-next.vercel.app/api/tempo-news';
...
async function fetchTempoNews(type = null) {
    // API eksternal menggunakan path parameter (contoh: /tempo-news/nasional)
    const url = type ? `${EXTERNAL_BASE_URL}/${type}` : EXTERNAL_BASE_URL;

    console.log(`[TEMPO NEWS] Mengambil data dari: ${url}`);
...
module.exports = function (app, prefix = '') {
    ...
    // GET /news/tempo
    // GET /news/tempo?type=nasional
    app.get(`${prefix}/news/tempo`, async (req, res) => {
        ...
        try {
            const { messages, cleanedData } = await fetchTempoNews(selectedType);

            const sourceName = selectedType ? `Tempo News (${selectedType})` : "Tempo News (All)";
            ...
            console.error(`[TEMPO NEWS API ERROR - ${selectedType || 'ALL'}]:`, error.message);
            
            const status = error.response ? error.response.status : 500;
            let errorMessage = "Kesalahan tak terduga saat mengambil daftar berita Tempo.";
